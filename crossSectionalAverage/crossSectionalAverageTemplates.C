...
template<class Type>
bool Foam::functionObjects::crossSectionalAverage::calcAverage()
{
  ...
  typedef GeometricField<scalar, fvPatchField, volMesh> volScalarFieldx;
    ...
    {
	volVectorFieldx U = lookupObject<volVectorFieldx>(fieldName_);
	...
	for (int i = 0; i < ndiv; i++)
	{
	    ...
	    {
		int celli = index + n;
		sum_UV = sum_UV + U_mag.internalField()[celli]*mesh_.V()[celli];
		sum_V = sum_V + mesh_.V()[celli];
	    }
	    sum_avg = sum_UV/sum_V;
	    for (int n = 0; n < ncells; n++)
	    {
    ...
    {
	volScalarField U_mag = lookupObject<volScalarFieldx>(fieldName_);
